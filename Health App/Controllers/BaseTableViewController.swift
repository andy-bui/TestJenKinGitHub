...
class BaseTableViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, BaseDataSourceDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let footerText = self.footerText {
            tableFooterView.frame = CGRect(x: 0, y: 0, width: tableView.frame.width, height: 44)
            let textLb = UILabel(frame: CGRect(x: 0, y: 0, width: tableView.frame.width - 30, height: 44))
            textLb.center = tableFooterView.center
