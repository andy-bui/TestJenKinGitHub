...
class HomeViewController:BaseViewController  {
    ...
    func setDataScreen(_ dataRecords: Results<DataRecording>) {
        ...
        if let caloPerDay = UserDataManager.sharedInstance.estimatedCaloPerDay(lastDataRC.bodyWeight, self.height){
        ...
        for dataRC in dataRecords{
            if let caloPerDay = UserDataManager.sharedInstance.estimatedCaloPerDay(dataRC.bodyWeight, self.height){
        ...
        if let remainCalo = remainTargetCalo.roundToString() {
            var str = "   目標まで \(remainCalo) kcal   "
            var attributedString = NSMutableAttributedString(string: str)
            let kNum:Int = remainCalo.description.characters.count
            attributedString.addAttribute(NSFontAttributeName, value: UIFont.systemFont(ofSize: 20), range: NSRange(location:9,length:kNum))
            reducedCaloIntake?.attributedText = attributedString
            
            
            str = "目標まで \(remainCalo) kcal"
            attributedString = NSMutableAttributedString(string: str)
            attributedString.addAttribute(NSFontAttributeName, value: UIFont.systemFont(ofSize: 20), range: NSRange(location:6,length:kNum))
            reducedCaloIntakeBiko?.attributedText = attributedString
        }
